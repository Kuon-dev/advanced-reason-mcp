...
export const GetGeminiThinkerSchema = z.object({
  originPrompt: z.string().describe("The user's request that requires reasoning analysis"),
  mode: z.enum([
    "reasoning_partner", 
    ...
    "compare_contrast", 
    "simulate_debate"
  ]).default("reasoning_partner").describe("The thinking mode to use"),
  output_count: z.number().optional().default(3).describe("Number of distinct perspectives or options to generate"),
  constraints: z.string().optional().describe("Specific requirements or focus areas to guide the thinking process")
});
...
export class GeminiServer {
  ...
  async getGeminiCompletion(args: z.infer<typeof GetGeminiThinkerSchema>): Promise<string> {
...
export const GEMINI_THINKER_TOOL: Tool = {
  name: "gemini-thinker",
  description: "A cognitive exploration tool that leverages Gemini's reasoning capabilities to analyze problems from multiple perspectives and generate structured thinking outputs. This tool is particularly valuable for complex reasoning tasks, comparing viewpoints, or performing structured analysis that reveals step-by-step thought processes.",
  inputSchema: {
    type: "object",
    properties: {
      originPrompt: {
        type: "string",
        description: "The user's original prompt or question that requires deep analysis. For best results, pass the complete prompt context without modification.",
      },
      mode: {
        type: "string",
        enum: ["reasoning_partner", "generate_perspectives", "brainstorm_options", "analyze_pros_cons", "compare_contrast", "simulate_debate"],
        description: "Optional mode specifying the type of thinking desired. Use 'reasoning_partner' for detailed step-by-step thought processes.",
        default: "reasoning_partner"
      },
      output_count: {
        type: "integer",
        description: "Optional number of distinct perspectives or options to generate. Defaults to 3.",
        default: 3
      },
      constraints: {
        type: "string",
        description: "Optional specific requirements or focus areas to guide the thinking process."
      }
    },
